...
def plot_time_differences_bar(time_info_list):
    ...
    limited_time_info_list = time_info_list[:50]
    
    package_names = [info['package_name'] for info in limited_time_info_list]
    time_differences = [info['time_difference'].total_seconds() / 3600 for info in limited_time_info_list]
    greater_than_24 = [info['greater_than_24'] for info in limited_time_info_list]
    
    plt.figure(figsize=(12, 8))
    colors = ['#ff3377' if gt24 else '#67ede7' for gt24 in greater_than_24]
    ...
    plt.title('Time Differences between Commit Finder Date and Maven Timestamp (Top 50 Packages)')
